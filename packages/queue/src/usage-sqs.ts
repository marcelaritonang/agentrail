...
type UsageSqsCommand =
  | SendMessageCommand
  | ReceiveMessageCommand
  | DeleteMessageCommand
  | ChangeMessageVisibilityCommand;

export type UsageSqsClientLike = {
  send(command: UsageSqsCommand): Promise<unknown>;
};

export function createSqsUsageEventQueue(options: {
  client: UsageSqsClientLike;
  queueUrl: string;
}): UsageEventQueue {
  return {
    async enqueue(batch) {

...
export function parseAgentRailCommand(argv: readonly string[]): AgentRailCommand {
  ...
  switch (command) {
    ...
    case "context": {
      ...
      if (tokenBudget === undefined) throw new Error("--token-budget is required");
      return {
        name: "context",
        root: stringFlag(flags, "root") ?? process.cwd(),
        task,
...
function clientsFromFlags(flags: Map<string, string[]>): readonly AgentRailClient[] {
